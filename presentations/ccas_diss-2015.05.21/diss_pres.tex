...
\begin{document}
	...
	\begin{frame}
		...
		\begin{Pred}[аналог теоремы о введении пространства оценок]\label{st:decompositon}
	...
	\begin{frame}
		...
		\par\bigskip
		\begin{Theorem}
			Алгоритм $\mathfrak A_{pm}$ корректен, т.~е. в конечной последовательности значений $\langle\tilde m^{*(0)},\tilde m^{*(1)},\dots\rangle$, которая строится с помощью алгоритма $\mathfrak A_{pm}$ для  значения $\tilde m^0$, полученного из внешней среды, расстояние до $\tilde m^0$ уменьшается.
		\end{Theorem}
	\end{frame}
