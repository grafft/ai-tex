...
\begin{document}
	...
	\begin{frame}
		...
		\onslide<1->{
			\node at (0,1.7) {Мозг};
			\node[align=center] at (0,0) {\includegraphics{phisio/mozg_2}};
			
			\node at (10,1.7) {Психические функции};
			\node[align=center] at (10,-0.2) {\includegraphics[width=0.3\textwidth]{signs/signs_net}};
			...
			\scriptsize
			\draw[-latex,thick,shorten >=2pt] (tmp1) edge[bend right] node[yshift=-18, left,align=center] {сенсорный\\вход} (1.4,-0.1);
		}
		...
		\onslide<2->{
			...
			\draw[-latex,thick,shorten >=2pt]  (3,-2) -- (3.2,-0.8);
			\draw[-latex,thick,shorten >=2pt] (3,-2) -- (5.0,-0.8);
			\draw[-latex,thick,shorten >=4pt] (3,-2) -- node[yshift=-7, right, align=center] {входы\\автоматов} (7.0,-0.8);
			...
			\draw[->] (tmp5) edge[bend left] (9,0.9);
			\draw[->] (tmp6) edge[bend left] (8.5,-0.1);		
		}
		\end{tikzpicture}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item на основе этих операторов исследовать автоматную функцию,
	...
	\begin{frame}[t]
		...
		\begin{itemize}
			\item вектор $\bar{x}_i^j(t)$ длины $q_i^j$ "--- входной сигнал (вектор весов входных признаков), 
	...
	\begin{frame}
		\frametitle{Алгоритм вычисления автоматной функции}
	...
	\begin{frame}
		...
		\par\bigskip
		Искомый оператор распознавания $\hat{R}$ должен выдавать весовую матрицу распознаваемых признаков $\gamma_{\Delta{t}}$, столбцы которой должны сходиться (с учётом корректного решающего правила) за $\tau_s+h$ шагов к информационному вектору $\bar\alpha$. 
	\end{frame}
	...
	\begin{frame}
		...
		\par\bigskip
		\begin{Theorem}
			Алгоритм $\mathfrak A_{pm}$ корректен, т.~е. последовательность значений $\langle\tilde m^{*(0)},\tilde m^{*(1)},\dots\rangle$, которая строится с помощью алгоритма $\mathfrak A_{pm}$ для  значения $\tilde m^0$, полученного из внешней среды, стремится к $\tilde m^0$.
		\end{Theorem}
	\end{frame}
