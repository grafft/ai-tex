...
\begin{document}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item 
			каждая вершина, принадлежащая $j$-ому ярусу графа $G_R$, является распознающим блоком $R_i^j$ уровня $j$,
			\item
			каждое ребро $e=(R_{i_1}^{j_1},R_{i_2}^{j_2}){\in}E$ обозначает иерархическую связь между дочерним блоком $R_{i_1}^{j_1}$ и блоком"--~родителем $R_{i_2}^{j_2}$.				
		\end{itemize}
	\end{frame}
	...
	\begin{frame}
		\frametitle{Вычислительный цикл распознающего блока}
	...
	\begin{frame}
		...
		В работе исследуется функционирование иерархии распознающих блоков со сформированными матрицами предсказания после завершения процесса обучения (например, по алгоритму HTM).
	...
	\begin{frame}
		...
		Для описания действий будем использовать правила, которые в искусственном интеллекте представляются в виде множества условий и эффектов. Введём два выделенных из множества $\{f_k\}$ признака: $f_c$ "--- ``условие'' и $f_e$ "--- ``эффект'', измеряемые распознающим блоком $R_0^1$.
		\begin{Def}
