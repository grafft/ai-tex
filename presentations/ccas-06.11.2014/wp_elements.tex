...
\begin{document}
	...
	\begin{frame}
		...
		Картина мира человека представляет собой сложное психофизиологическое образование, позволяющее ему как субъекту деятельности на материальном субстрате головного мозга строить модель действительности и на основе этой модели выполнять различные психические функции: 
	...
	\begin{frame}
		\frametitle{Современные представленяи о принципах работы коры головного мозга}
		\begin{columns}
	...
	\begin{frame}
		\frametitle{Протознак и именование знака}
		До того, как происходит связывание компонент знака в единую структуру под одним именем, существуют лишь ``парные'' переходы между компонентами знания агента о том или ином явлении. До моментам именования эти компоненты образуют ``протознак'':
		\begin{itemize}
		...
		\only<2>{
			\begin{algorithm}[H]
				...
				\begin{algorithmic}[1]
					...
					\Return $\Psi_p^m$, определенная на паре $(\tilde p, \tilde m)$, где $\tilde p=\lim\limits_{t\rightarrow\infty}\tilde p^{*(t)}$, 
	...
	\begin{frame}
		\frametitle{Доказательство теоремы корректности}
		\begin{proofs}
	...
	\begin{frame}
		...
		\begin{proofs}[\proofname\ (продолжение)]
			Так как выполнено условие шага 1, то признак $\tilde f_p^{(t+1)}$ выполним на некотором $(2\cdot s-1$-м столбце условий матрицы предсказания признака $f_p$. Это означает, что матрицы $Z_p^{(t)}$ и $Z_p^{(t+1)}$ будут отличать только в двух вектор-столбцах $(2\cdot s-1)$-м и $(2\cdot s)$-м:
			\begin{equation}
			\bar z_{2\cdot s-1}^{c(t+1)}=\bar z_{2\cdot s-1}^{c(t)}\vee (\bar z^{c(t+1)}\rightarrow F_p^j),
			\end{equation}
			\begin{equation}
			\bar z_{2\cdot s}^{e(t+1)}=\bar z_{2\cdot s}^{e(t)}\vee(\bar z^{e(t+1)}\Rightarrow \bar z_{2\cdot s}^e).
			\end{equation}
	...
	\begin{frame}
		\frametitle{Архитектура интеллектуального агента}	
		\begin{figure}[t]
			\includegraphics[width=0.5\linewidth]{iagent}
		\end{figure}
	\end{frame}	
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			\item Построен алгоритм итерационного процесса формирования нового элемента картины мира.
			\item Проведено оригинальное исследование итерационного процесса формирования нового элемента картины мира.
		\end{itemize}
	\end{frame}
