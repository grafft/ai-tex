...
%
\title{The Guide for Writing \LaTeX\ Documents \\
       for Procedia Publication in EasyChair}
...
\keywords{EasyChair, Procedia, significance, conference management}
...
\begin{abstract}
  ...
  proceedings. This document describes version 3.4 of the style and
  its adaptation for the Procedia publication series by Elsevier Science.
\end{abstract}
...
The styles and parameters of this guide are designed for compliance
with the requirements for publication in the EasyChair conference
system \cite{easychair}, and are also applicable to the Procedia
publications series by Elsevier Science.
...
system \cite{easychair}, serving thousands of conferences.
...
The use of EasyChair and this style for creating Procedia volumes is a
pilot project between Elsevier Science and EasyChair.

EasyChair accepts documents written in any of the following formats:
\begin{itemize}
\item \LaTeX;
\item Word documents (.docx), which can be produced by Microsoft Word;
\item the Open Document Text (.odt) format, which can be produced by
LibreOffice or OpenOffice.
\end{itemize}
This guide explains how to produce documents in \LaTeX. To achieve
high quality of volumes, both LaTeX and Word documents should have the
same layout and similar styles. This guide is provided for the users
of \LaTeX\ and describes EasyChair style version 3.4.
...
The Procedia conferences should use the \texttt{procedia} option with
the document class command:
\verb+\documentclass[procedia]{easychair}+. When you use this option,
the top of the first page will be reserved for a page header
containing Elsevier Science and Procedia logos and the volume
information. This header will be added to the article when you submit
it to EasyChair. You cannot control this header placement and for this
reason should not try to change vertical spacing on the first page.

The class uses 10pt fonts. No attempt was made to adapt it to 11pt or
12pt fonts, so using options changing the font size is not recommended.
...
document class entry,
e.g.:\\\verb+\documentclass[a4paper]{easychair}+, however this is not
recommended for Procedia papers.
...
invisible or hardly noticeable for people and overflows by
...
\begin{itemize}
\item
{\easychair} version 3.2--3.4, version 3.4 made in February 2014. The
main changes are in the \verb|\author| and \verb|institute| commands
and the general layout of the first page.
