	...
	\Function{Planning}{$Plan$, $F_{cur}$, $A_{forw}$}
		...
		\Else
			...
			\Else
				\State $\Delta=F_{next}\setminus F_{st}$; \Comment{текущая невязка состояний}					
				...
				\ForAll $a_j\in A_{next}$
					\If{$\exists a_k\in A_{next}$ такой, что $a_k\not =a_i$ и $a_k$ конфликтует с $a_j$}
						\State $a_{del}=\argmin\limits_{a\in\{a_k,a_j\}}|F_A(a)\setminus\Delta|$;
						\State $A_{next}= A_{next}\setminus\{a_{del}\}$; \Comment{удаляем конфликтующие признаки}
					\EndIf
				\EndFor
