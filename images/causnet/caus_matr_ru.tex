...
\begin{document}
	\begin{tikzpicture}[join=round,scale=2.0]
		...
		\node[align=center] at (120pt, 50pt) {\large Компонента\\\large знака};
		...
		\node at (30 pt, 95 pt) {$i$};
		\node at (60 pt, 95 pt) {$-1$};
		\node at (80 pt, 95 pt) {$-j$};
		
		\node at (10 pt, 5 pt) {$e_1$};
		\node at (30 pt, 5 pt) {$e_i$};
		\node at (60 pt, 5 pt) {$e_{i+1}$};
		\node at (80 pt, 5 pt) {$e_{i+j}$};
		
		\node at (25 pt, -5 pt) {\large Причины};
		\node at (75 pt, -5 pt) {\large Эффекты};
				
		\node[ell, ellipse] (s1) at (0, -40pt)  {Зак 1};
		\node[ell, ellipse] (s2) at (10pt, -20pt)  {Знак 2};
		\node[ell, ellipse] (s3) at (30pt, -30pt)  {Знак 3};
		\node[ell, ellipse] (s4) at (70pt, -30pt)  {Знак 4};
