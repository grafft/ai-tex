...
\begin{document}
	\begin{tikzpicture}[join=round,scale=2.0]
		\tikzstyle{ell}=[draw, thick, align=center, font=\small]
		...
		\node at (10 pt, 95 pt) {$1$};
		\node at (30 pt, 95 pt) {$t_c$};
		\node at (60 pt, 95 pt) {$-1$};
		\node at (80 pt, 95 pt) {$-t_e$};
		
		\node at (10 pt, 5 pt) {$e_1$};
		\node at (30 pt, 5 pt) {$e_{t_c}$};
		\node at (60 pt, 5 pt) {$e_{t_c+1}$};
		\node at (80 pt, 5 pt) {$e_h$};
		
		\node at (25 pt, -5 pt) {\large Причины};
		\node at (75 pt, -5 pt) {\large Эффекты};
				
		\node[ell, ellipse] (s1) at (0, -40pt)  {признак 1};
		\node[ell, ellipse] (s2) at (10pt, -20pt)  {признак 2};
		\node[ell, ellipse] (s3) at (25pt, -31pt)  {признак 3};
		\node[ell, ellipse] (s4) at (70pt, -30pt)  {признак 4};
		...
		\draw[ell, ->,rounded corners] (p8.east) -| (95pt, 0) node[yshift=-30pt,above, rotate=90] {\small $conn_5$} |- (s4);
	\end{tikzpicture}
\end{document}