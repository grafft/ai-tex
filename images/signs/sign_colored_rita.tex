...
\begin{document}
	...
	\begin{tikzpicture}[join=round,scale=3.0,node distance = -0.05]
		...
		\node[left = of s_p] {$p$};
		\node[below = of s_p,align=center] {Image\\(cover,pages)};
		\node[right = of s_m] {$m$};
		\node[label,node distance = 0.01, xshift=5mm, above = of s_m,align=center] {Significance\\(read, buy))};
		\node[right = of s_n] {$n$};
		\node[left = of s_n, align=center] {Sign name\\(book))};
		\node[right = of s_a] {$a$};
		\node[below = of s_a, xshift=10mm, align=center] {Persoanl meaning\\(reading, stand for mug))};
