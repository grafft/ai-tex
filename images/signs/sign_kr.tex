...
\begin{document}
	...
	\begin{tikzpicture}[join=round,scale=3.0,node distance = -0.05]
		...
		\node[align=center,font=\footnotesize,fill=white,opacity=0.8,text opacity=1] at (-25pt,-12pt) {procedural\\features $Z_m$};		
		...
		\node[align=center,font=\footnotesize,fill=white,opacity=0.8,text opacity=1] at (50pt,30pt) {sets of\\features $Z_p$};
		...
		\node[align=center,font=\footnotesize,fill=white,opacity=0.8,text opacity=1] at (50pt,-15pt) {procedural\\features $Z_a$};		
		
		\opredictors{a1}{30}{50pt}{-40pt}
		
		\ppredictors{a2}{30}{20pt}{-40pt}
		...
		\node[align=center,font=\scriptsize] at ($(z_l_a)+(22pt,-8pt)$) {feature $f_2$\\of the action\\effect};
		\node[align=center,font=\scriptsize] at ($(z_l_a)+(-25pt,-6pt)$) {feature $f_1$\\of the action\\condition};
		\node[draw, circle, fill=white, scale=0.5] at ($(z_l_a)+(3pt,-3.5pt)$) {};
		
		\node[align=center,font=\small] (ppo) at (-20pt,-35pt) {path planning\\operations};
		\draw[->,rounded corners] ($(z_l_a2)+(-4pt,-1pt)$) -| (ppo.south);
		\node[draw, circle, fill=white, scale=0.5] at ($(z_l_a2)+(-3pt,-1pt)$) {};
		
		\node[align=center,font=\footnotesize,fill=white,opacity=0.8,text opacity=1] at (7pt,-35pt) {ction\\matrices of\\feature $f_1$};
		\node[align=center,font=\footnotesize,fill=white,opacity=0.8,text opacity=1] at (37pt,-32pt) {procedural features\\of the significance of\\the sign $f_2$};
	\end{tikzpicture}
