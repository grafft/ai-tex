...
\usepackage{tikz}
\usetikzlibrary{shapes,positioning,calc,backgrounds}

% TikZ styles for drawing

\begin{document}
	
	\begin{tikzpicture}[join=round,scale=3.0,node distance = -0.05,show background rectangle,
	background rectangle/.style={fill=white}]
		...
		\node[sign_comp, fill=green!60!black] (s_a) at (0.7,0.0) {};
		
		\node[left = of s_p] {$p$};
		\node[below = of s_p] {Образ};
		\node[right = of s_m] {$m$};
		\node[label,node distance = 0.01, above = of s_m] {Значение};
		\node[right = of s_n] {$n$};
		\node[left = of s_n,align=center] {Имя\\знака};
		\node[right = of s_a] {$a$};
		\node[below = of s_a,align=center] {Личностный\\смысл};
