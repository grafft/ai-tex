...
\begin{document}
	...
	\begin{tikzpicture}[>=triangle 60]
		...
		\node[level_rect, minimum height = 15mm] (level_3) at (0, 2.5) {};
		\node[font=\huge] at(0, 2.5) {Reactive level};		
		
		\draw[->,rounded corners] ([xshift=-3mm]pred_tr.south) node[align=center,yshift=-7mm,left] {movement\\parameters}|- ($(pred_tr.south)+(-7mm,-20mm)$) -| (-5.7, 3.3);
		\draw[->,rounded corners] (3, 3.3) node[align=center, yshift = 5mm, left] {geometric\\constraints} |-  (4, 4) -| ([xshift=10mm]build_tr.south);
		\draw[->,rounded corners] (build_tr.east) -| node[align=center,yshift=-50mm,right] {desired path} (6.7, 3.3);
		\draw[->,rounded corners] (-1, 3.3) node[align=center, yshift = 7mm, left] {threshold was\\exceeded or not} |- (0, 4.7) -|  ([xshift=10mm]monitor.south);
		...
		\draw[->,rounded corners] (sensors.east) node[align = center, xshift = 30mm, above] {phase coordinates} -| (0, 1.7);
