...
\begin{document}
	...
	\begin{tikzpicture}[scale=2.0]
		...
		\node[align=center, font=\small] at (30pt,-25pt) {S-step};
		
		\node[align=center, font=\small] (ppo) at (10pt,-50pt) {path planning\\operations};
		\draw[->] (sit_3) to[out=-180, in = -90] ([xshift=-5pt]ppo.south);
		\draw[->] ([xshift=5pt]ppo.south) to[out=-90, in = 90] node[font=\small,right,yshift=10pt,xshift=-7pt] {success} (sit_3);
		
		\draw[->] ([xshift=-5pt]ppo.north) to[out=90,in=-90] node[align=center,left, font=\small]{deteced\\obstacles} (sit_st);
		
		\draw[->] (sit_3) to[out=30, in = -50] (sit_st);
	\end{tikzpicture}
