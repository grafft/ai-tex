...
\begin{document}
	...
	\begin{tikzpicture}[join=round,scale=3.0,node distance = -0.05]
		...
		\node[align=center]	at ($(s2_m)+(-1.1,0.6)$) {целевой знак};
				
		\draw[->,thick, color = blue] (ss_m) edge [bend left, out = 80, in = 90] node[yshift=4,auto,align=center]{сценарное\\отношение} (s1_m);
		\draw[->,thick, color = blue] (s1_m) edge [bend left, out = 80, in = 90] node[yshift=4,auto,align=center]{сценарное\\отношение} (s2_m);
