...
\begin{document}
	\begin{tikzpicture}
		\tikzstyle{z_matrix} = [draw, rectangle, minimum width = 60, minimum height = 60,fill=white];
			...
			\draw[->, thick, green!60!black] ($(z_1)+(-0.1,-3)$) -- node[right,black] {$\bar x^j(0)$} ($(z_1)+(-0.1,-2)$);
			...
			\draw[->, ultra thick] ($(z_1)+(2.6,-0.6)$) -- node [above] {\scriptsize$\frac{\| e_1-\bar x^j(0)\|}{\|e_1\|+\|\bar x^j(0)\|}$} ($(z_1)+(3.7,-0.6)$);
			...
			\draw[->, thick, red] ($(z_2)+(0.3,1.4)$) -- node[right,black] {$\tilde x^j(0)$} ($(z_2)+(0.3,3)$);
			...
			\draw[->, thick, green!60!black] ($(z_2)+(0.3,-3)$) -- node[right,black] {$\bar x^j(1)$} ($(z_2)+(0.3,-2)$);
			
			\draw[->, ultra thick] ($(z_2)+(2.6,-0.6)$) -- node [above] {\scriptsize$\frac{\|e_2-\bar x^j(1)\|}{\|e_2\|+\|\bar x^j(1)\|}$} ($(z_2)+(3.7,-0.6)$);
