...
\begin{document}
	...
	\begin{tikzpicture}[join=round,scale=3.0,node distance = -0.05]
		...
		\node[label_small] at ($(s_p)+(-0.45,0.85)$) {распознающий\\блок};
		...
		\node[align=center,font=\footnotesize,fill=white,opacity=0.8,text opacity=1] at ($(s_m)+(0.27,1.0)$) {распознающий блок\\экстериоризованных\\действий};
		...
		\node[align=center,font=\footnotesize,fill=white,opacity=0.8,text opacity=1] at ($(s_a)+(0.7,-0.2)$) {распознающий блок\\интериоризованных\\действий};
				
		\node[left = of s_p, font=\large] {Образ};
		\node[right = of s_m, font=\large]{Значение};
		\node[above left = of s_n,align=center, font=\large] {Имя\\знака};
